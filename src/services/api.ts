import axios from "axios";

const baseAPI = axios.create({
  baseURL: "http://localhost:5001/",
});

interface UserData {
  email: string;
  password: string;
}

function getConfig(token: string) {
  return {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  };
}

async function signUp(signUpData: UserData) {
  await baseAPI.post("/sign-up", signUpData);
}

async function signIn(signInData: UserData) {
  return baseAPI.post<{ token: string }>("/sign-in", signInData);
}

export interface Term {
  id: number;
  number: number;
}

export interface Discipline {
  id: number;
  name: string;
  teacherDisciplines: TeacherDisciplines[];
  term: Term;
}

export interface TeacherDisciplines {
  id: number;
  discipline: Discipline;
  teacher: Teacher;
  tests: Test[];
}

export interface Teacher {
  id: number;
  name: string;
}

export interface Category {
  id: number;
  name: string;
}

export interface Test {
  id: number;
  name: string;
  pdfUrl: string;
  category: Category;
  views: number;
}

export type TestByDiscipline = Term & {
  disciplines: Discipline[];
};

export type TestByTeacher = TeacherDisciplines & {
  teacher: Teacher;
  disciplines: Discipline[];
  tests: Test[];
};

async function getTestsByDiscipline(token: string, query: string = "") {
  const config = getConfig(token);
  return baseAPI.get<{ tests: TestByDiscipline[] }>(
    `/tests?groupBy=disciplines&search=${query}`,
    config
  );
}

async function getTestsByTeacher(token: string, query: string = "") {
  const config = getConfig(token);
  return baseAPI.get<{ tests: TestByTeacher[] }>(
    `/tests?groupBy=teachers&search=${query}`,
    config
  );
}

async function getCategories(token: string) {
  const config = getConfig(token);
  return baseAPI.get<{ categories: Category[] }>("/categories", config);
}

async function getDisciplines(token: string) {
  const config = getConfig(token);
  return baseAPI.get<{ disciplines: Discipline[] }>("/disciplines", config);
}

async function getTeachersByDiscipline(token: string, id: number) {
  const config = getConfig(token);
  return baseAPI.get<{ teachers: Teacher[] }>(`/teachers?id=${id}`, config);
}

export interface TestCreationData {
  name: string;
  pdfUrl: string;
  categoryId: number;
  teacherId: number;
  disciplineId: number;
}

async function createTest(token: string, testData: TestCreationData) {
  const config = getConfig(token);
  return baseAPI.post("/app/adicionar", testData, config);
}

async function addView(token: string, id: number) {
  const config = getConfig(token);
  return baseAPI.put(`/app/view?id=${id}`, undefined, config);
}

async function searchTest(
  token: string,
  target: "disciplines" | "teachers",
  query: string
) {
  const config = getConfig(token);
  return baseAPI.get(`/search/${target}?search=${query}`, config);
}

const api = {
  signUp,
  signIn,
  getTestsByDiscipline,
  getTestsByTeacher,
  getCategories,
  getDisciplines,
  getTeachersByDiscipline,
  createTest,
  addView,
  searchTest,
};

export default api;
